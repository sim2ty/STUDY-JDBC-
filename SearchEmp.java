...
public class SearchEmp {
	public static void main(String[] args) throws Exception{
		...
		while(true) {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT ename,to_char(hiredate,'yyyy\"년 \"mm\"월\"') 입사시기,deptno from emp WHERE lower(ename) = ?");
			pstmt.setString(1, text);
			int find_su = pstmt.executeUpdate();

			if(find_su>=1) {
				rs = pstmt.executeQuery();
				while (rs.next()) {
					System.out.println(rs.getString("ename") + " 직원은 근무중입니다.");
					System.out.println(rs.getString("입사시기")+"에 입사했고 현재 " + rs.getString("deptno") + " 부서에서 근무하고 있습니다.");
				}
				pstmt.close();
				conn.close();
				break;
